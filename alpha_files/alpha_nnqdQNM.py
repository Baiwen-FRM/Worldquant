...
# regression_neut(regression_neut(regression_neut(sign(UTR)*power(abs(UTR),0.5),turn20),vwap),ts_delta(mdl175_retainedearningsps/sharesout,120))
#
...
def regression_neut(y, x):
    """
    Cross-sectional regression: regress y on x, return residual.
    regression_neut(y, x) = y - beta * x
    ...
    xv = x[valid]
    yv = y[valid]
    beta = np.nansum(xv * yv) / max(np.nansum(xv * xv), 1e-30)
    residual = y - beta * x
...
@alpha(
    data=["volume", "sharesout", "cap", "vwap", "mdl175_retainedearningsps"],
    store=[
        {"name": "turn_buffer", "dims": "xi", "extend": np.float64(np.nan)},
        {"name": "md_sharesout_buffer", "dims": "xi", "extend": np.float64(np.nan)},
    ],
)
def alpha_fn(data, store) -> npt.NDArray[np.float32]:
    ...
    md_f = field_to_float(data.mdl175_retainedearningsps[-1])
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        md_ratio = np.divide(md_f, sharesout_f, out=np.full(n, np.nan), where=sharesout_f != 0)
    if store.md_sharesout_buffer is None:
        store.md_sharesout_buffer = md_ratio[np.newaxis, :]
    else:
        store.md_sharesout_buffer = np.vstack([store.md_sharesout_buffer, md_ratio[np.newaxis, :]])[-WINDOW_TS_DELTA:]
    ...
    signal = regression_neut(signal, ts_delta_from_buffer(store.md_sharesout_buffer))
